...
# @Date:   2021-02-08T17:12:08+01:00
# @Email:  massimo.demauri@protonmail.com
# @Filename: C_interface.jl
# @Last modified by:   massimo
# @Last modified time: 2021-02-11T18:26:09+01:00
# @License: LGPL-3.0
# @Copyright: {{copyright}}

# code originally from Clp.jl version 0.7.2
...
function load_problem(model::ClpModel,num_cols::Integer,num_rows::Integer,
                      start::Vector{CoinBigIndex},index::Vector{Int32},value::Vector{Float64},
                      col_lb::VecOrNothing,col_ub::VecOrNothing,
                      obj::VecOrNothing,row_lb::VecOrNothing,row_ub::VecOrNothing)
...
function load_problem(model::ClpModel,obj::VecOrNothing,col_lb::VecOrNothing, col_ub::VecOrNothing,
                      cns_matrix::AbstractMatrix,row_lb::VecOrNothing,row_ub::VecOrNothing)

    mat = convert(SparseMatrixCSC{Float64,Int32},cns_matrix)
...
function load_problem(model::ClpModel,obj::VecOrNothing,col_lb::VecOrNothing, col_ub::VecOrNothing,
                      cns_matrix::SparseMatrixCSC{Float64},row_lb::VecOrNothing,row_ub::VecOrNothing)

    mat=cns_matrix
    # We need to convert to zero-based, but
    # TODO: don't make extra copies of arrays
    # TODO: check dimensions match
    load_problem(model, mat.n, mat.m, convert.(Int32,mat.colptr .- 1),
                 convert.(Int32,mat.rowval .- 1), mat.nzval,
                 col_lb, col_ub,obj, row_lb, row_ub)
end
...
function load_quadratic_objective(model::ClpModel,
    hessian_matrix::SparseMatrixCSC{Float64})
    load_quadratic_objective(model, hessian_matrix.n, convert.(Int32,hessian_matrix.colptr .- 1), convert.(Int32,hessian_matrix.rowval .- 1),hessian_matrix.nzval)
end
...
# Delete rows.
function delete_rows(model::ClpModel, which::Vector{Integer})
    _jl__check_model(model)
    @clp_ccall deleteRows Cvoid (Ptr{Cvoid},Int32,Ptr{Int32}) model.p length(which) Int32.(which.-1)
end

# Add rows.
function add_rows(model::ClpModel,A::SparseMatrixCSC{Float64,Integer},row_lb::VecOrNothing,row_ub::VecOrNothing)
    add_rows(model,size(A,1),row_lb,row_ub,convert.(Int32,A.colptr .- 1),convert.(Int32,A.rowval .- 1),A.nzval)
end
...
# Delete columns.
function delete_columns(model::ClpModel, which::Vector{Integer})
    _jl__check_model(model)
    @clp_ccall deleteColumns Cvoid (Ptr{Cvoid},Int32,Ptr{Int32}) model.p length(which) Int32.(which.-1)
end
...
function get_cns_matrix(model::ClpModel)
    _jl__check_model(model)
